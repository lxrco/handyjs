...
class BaseClass {
  ...
  /* Like load but instead of failing if not finding the object, it just reject the promise
   */
  load_e(identifier=[]){
    return new Promise((resolve, reject) => {
  ...
  findAll(){
    return new Promise((resolve, reject)=>{
      ...
      pool.getConnection((err, connection)=>{
        ...
        const query = 'SELECT * FROM ' + connection.escapeId(this.tableDefinition.name);
        connection.query(query, (err, results)=>{
          connection.release();
  ...
  return new Promise((resolve, reject)=>{
    ...
    pool.getConnection((err, connection)=>{
      ...
      connection.query(query, (err, results)=>{
        ...
        if(!results.length){return reject(errors.object_not_found) }
